...
def parse_game_state(obs: str) -> "GameState | None":
    ...
    # Pair. LP-label-fix: accept BOTH the env-server label ("Hand: Pair") and the
    # validator/reformatted label ("Hand: PAIR"). get_expert_action parses the
    # reformatted (validator-format) message, so the old exact "Hand: Pair" match
    # never fired and has_pair was always False. Case-insensitive covers both.
    has_pair = re.search(r"Hand:\s*Pair", obs, re.IGNORECASE) is not None

    # Round. Env-server emits "Current round: X/2"; reformat_lp_to_pvp rewrites it
    # to "Round: X/2". Accept either, else round silently defaulted to 1.
    round_str = _find(r"(?:Current round|Round):\s*(\d+)/\d+", "1")
    round_ = int(round_str)

    # Chips. Env-server "Pot size: N"; validator "Pot: N chips". Accept either.
    pot      = int(_find(r"Pot(?:\s*size)?:\s*(\d+)", "0"))
    ...
    def _parse_betting(label: str) -> list[str]:
        # Env-server "Round N betting:"; reformat_lp_to_pvp rewrites to
        # "Round N actions:". Accept either so opp_raised detection works.
        m = re.search(rf"{label} (?:betting|actions):\s*(.+)", obs)
        if not m:
            return []

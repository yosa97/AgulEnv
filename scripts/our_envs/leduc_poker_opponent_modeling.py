...
def parse_game_state(obs: str) -> "GameState | None":
    ...
    # LP-label-fix: accept BOTH env-server labels and validator/reformatted labels
    # (tolerant regex works regardless of which format is fed to this parser).
...
def _select_fallback_action(gs: "GameState | None") -> str:
    ...
    if gs is None or not gs.legal_actions:
        # audit P2: "0" = Fold (and illegal when no bet pending) — the SFT twin
        # fixed this exact bug; prefer Call/Check like leduc_poker_trajectories.
        return "1"

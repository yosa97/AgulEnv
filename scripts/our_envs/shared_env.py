...
# ---------------------------------------------------------------------------
# Reasoning-tag stripping (shared across opponent-modeling variants)
# ---------------------------------------------------------------------------
...
def remove_reasoning_tags(text: str) -> str:
    """
    Strip <think>...</think>-style blocks from a completion string.

    Required for reasoning-mode tokenizers (detected by
    ``model_utility.is_reasoning_tokenizer``). Without stripping, completions
    like ``<think>…</think>3`` fail ``int()`` parsing and every action is
    registered as invalid.

    Handles paired tags, orphan closers (keep content after last ``</tag>``),
    and orphan openers (keep content before first ``<tag>``).
    """
    cleaned = text
